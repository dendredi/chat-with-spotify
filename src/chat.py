...
examples = [
    HumanMessage(
        "Can you play the song that goes like [example lyrics]?", name="example_user"
    ),
    AIMessage(
        "",
        ...
        tool_calls=[
            {
                "name": "search_song_by_lyrics",
                "args": {"lyrics": "[example lyrics]"},
                "id": "1",
            }
        ],
    ),
    ToolMessage(
        Song(title="[example song]", artist_names="[example artist]"), tool_call_id="1"
    ),
    AIMessage(
        "",
    ...
    HumanMessage(
        "Can you play the song in which MJ was a zombie?", name="example_user"
    ),
    AIMessage(
...
def spotipy_request(function_name: str, parameters: dict | None = None) -> dict:
    ...
    try:
        return (
            spotipy_request_impl(function_name)
